...
if [ $db -eq 1 ]; then
	docker run --name lep-db -p 3306:3306 -e MYSQL_ROOT_PASSWORD=adminpwd -e MYSQL_USER=lportal -e MYSQL_PASSWORD=lportal -e MYSQL_DATABASE=lportal --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci -d mysql:5.6
fi
